...
const MyBlogs = () => {
  const { isAdminAuthenticated, adminId } = React.useContext(AdminContext);
  ...
  useEffect(() => {
    if (isAdminAuthenticated) {
      // Filter blogs for admin
      const adminBlogs = blogs.filter(blog => blog.author._id === adminId);
      setFilteredBlogs(adminBlogs);
    }
  }, [blogs, adminId, isAdminAuthenticated]);
    ...
    <div className="bg-white font-[sans-serif] my-4 pb-20">
      <div className="max-w-7xl mx-auto">
        <div className="text-start">
          <h2 className="text-3xl text-[#333] inline-block relative after:absolute after:w-4/6 after:h-1 after:left-0 after:right-0 after:-bottom-4 after:mx-auto after:bg-blue-400 after:rounded-full">My Admin Blogs</h2>
        </div>
